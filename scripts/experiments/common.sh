...
function dump_bucket {
  ...
  mc cp --recursive ${LOAD_BUCKET}/logs ${OUT_DIR}/${test_name}/
  mc cp --recursive ${LOAD_BUCKET}/results ${OUT_DIR}/${test_name}/
  empty_bucket
  myecho "-------------------"
}
...
function run_4_fold_exp {
  ...
  set_user_policy
  empty_bucket
  export exp_name="${TIMESTAMP}_${LOAD_NAME}_${POLICY}"
  signal_exp "start"
  run_serv_acct
  run_serv_noacct
  run_noserv_acct
...
function run_simple_test {
  TIMESTAMP=$(date "+%m_%d_%H:%M")
  export OUT_DIR="${EXP_OUT_DIR}/${TIMESTAMP}"
  mkdir -p ${OUT_DIR}
  set_user_policy
  empty_bucket
  export exp_name="${TIMESTAMP}_${LOAD_NAME}_${POLICY}"
